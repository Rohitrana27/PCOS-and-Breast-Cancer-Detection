...
#
lbl = tk.Label(root, text="Breast Cancer Detection", font=('times', 35,' bold '), height=1, width=60,bg="light blue",fg="black")
lbl.place(x=0, y=0)


frame_display = tk.LabelFrame(root, text=" --Display-- ", width=900, height=400, bd=5, font=('times', 14, ' bold '),bg="light blue")
frame_display.grid(row=0, column=0, sticky='nw')
frame_display.place(x=300, y=125)
...
frame_alpr = tk.LabelFrame(root, text=" --Process-- ", width=220, height=400, bd=5, font=('times', 14, ' bold '),bg="light blue")
frame_alpr.grid(row=0, column=0, sticky='nw')
frame_alpr.place(x=20, y=120)
...
#    global fn
    ...
    if fn!="":
        ...
        breast=np.argmax(prediction)
        print(breast)
        
        
        
        if breast == 0:
            Cd="Normal(NO-PCR) Breast Detected"
        elif breast == 1:
            Cd="Abnormal(PCR) breast Detected"
    A = Cd 
...
def openimage():
    ...
    global fn
    fileName = askopenfilename(initialdir='D:/Final code/100% code breast cancer', title='Select image for Aanalysis ',
                               filetypes=[("all files", "*.*")])
    IMAGE_SIZE=200
    imgpath = fileName
    fn = fileName
...
def file():
    ...
    call(["python","breast precaution.py"])
def window():
    root.destroy()
...
#
button4 = tk.Button(frame_alpr, text="Precaution", command=file,width=15, height=1,bg="white",fg="black", font=('times', 15, ' bold '))
